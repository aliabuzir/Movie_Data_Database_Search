...
def select_one_row(dbConn, sql, parameters = None):
  # If parameters equals None, we want to assign it as an empty list because .execute() needs a list for the second parameter.
  if (parameters == None):
    parameters = []

  # Cursor object to point to database and retreive requested data.
  dbCursor = dbConn.cursor()

  # We attempt the execution and fetch process, if an error occurs we output an error message, then close(deallocate) the cursor object.
  try:
    dbCursor.execute(sql, parameters)
    row = dbCursor.fetchone()
    # If NO data is received from fetch.
    if (row == None):
      return ()
    # If data is received from fetch.
    else:
      return row
...
def perform_action(dbConn, sql, parameters = None):
  ...
  # We attempt to execute the query and commit changes and return rows modified, if a failure occurs then we output the failure message and return -1 to the call to represent a failure in action process.
  try:
    dbCursor.execute(sql, parameters)
    dbConn.commit()
